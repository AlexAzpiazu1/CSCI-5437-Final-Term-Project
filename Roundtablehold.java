...
public class RoundtableHold extends JFrame {
    ...
    private Shape3D[] puzzle1Answers = new Shape3D[4];
    // -1 = unanswered, 0-3 = index of chosen answer
    private int puzzle1ChosenAnswer = -1;
    ...
    private BranchGroup buildScene(SimpleUniverse universe, Canvas3D canvas, TransformGroup vpTG) {
        ...
        canvas.addMouseListener(new PuzzleClickHandler(canvas, root));
    ...
    private Transform3D hiddenKeyTransform(int index) {
        Transform3D t = new Transform3D();
        t.setScale(0.001);
        t.setTranslation(new Vector3f(KEY_POSITIONS[index].x, -100f, KEY_POSITIONS[index].z));
        return t;
    }
    ...
    private void spawnKey(int index) {
        ...
        keyTGs[index].setTransform(visibleKeyTransform(index));
    }

    private void solvePuzzle(int puzzleIndex) {
        if (puzzleIndex < 0 || puzzleIndex >= puzzleSolved.length)
            return;
        ...
        if (puzzleIndex == 0) {
            ...
            if (puzzle1PaperShape != null) {
                puzzle1PaperShape.setAppearance(puzzlePaperAppearance(puzzle1ChosenAnswer));
            }
        }
    }

    private void answerPuzzle1Wrong(int chosenIndex) {
        // Allow re-answering only if not yet solved
        if (puzzleSolved[0]) return;
        puzzle1ChosenAnswer = chosenIndex;
        if (puzzle1PaperShape != null) {
            puzzle1PaperShape.setAppearance(puzzlePaperAppearance(puzzle1ChosenAnswer));
        }
    }
    ...
    private TransformGroup buildPuzzleBoards() {
        ...
        Appearance paperApp = puzzlePaperAppearance(-1);
        ...
        for (int i = 0; i < 4; i++) {
            Shape3D hitbox = new Shape3D(
                    quadGeo(
                            p(centerX - answerW / 2f, answerYs[i] - answerH / 2f, z - 0.02f),
                            p(centerX + answerW / 2f, answerYs[i] - answerH / 2f, z - 0.02f),
                            p(centerX + answerW / 2f, answerYs[i] + answerH / 2f, z - 0.02f),
                            p(centerX - answerW / 2f, answerYs[i] + answerH / 2f, z - 0.02f),
                            new float[] { 0, 0, -1 }),
                    invisiblePickAppearance());

            // Store answer index (0-3) as userData; index 1 is the correct answer (b)
            hitbox.setUserData(i);
            hitbox.setCapability(Shape3D.ALLOW_APPEARANCE_WRITE);
            hitbox.setPickable(true);
            puzzle1Answers[i] = hitbox;
            group.addChild(hitbox);
        }
    ...
    private void collectKey(int index) {
        ...
        keyTaken[index] = true;
        collectedKeys++;
        keyTGs[index].setTransform(hiddenKeyTransform(index));
    ...
    // =========================================================================
    private class PuzzleClickHandler extends MouseAdapter {
        private final PickCanvas pickCanvas;
        private final Canvas3D canvas;

        PuzzleClickHandler(Canvas3D canvas, BranchGroup root) {
            this.canvas = canvas;
            // BOUNDS mode is more reliable than GEOMETRY for clicking near-transparent flat quads
        ...
        public void mouseClicked(MouseEvent e) {
            // Always pick from screen centre (crosshair), not mouse position
            pickCanvas.setShapeLocation(canvas.getWidth() / 2, canvas.getHeight() / 2);
            ...
            int chosenIndex = (Integer) data;

            // Check whether this shape is one of our answer hitboxes
            boolean isAnswer = false;
            for (Shape3D ans : puzzle1Answers) {
                if (ans == node) { isAnswer = true; break; }
            }
            if (!isAnswer) return;
            ...
            if (chosenIndex == 1) {
                ...
                // Wrong answer — show X and allow retry
    ...
    private class FirstPersonController extends Behavior implements KeyListener, MouseMotionListener {
        ...
        FirstPersonController(TransformGroup viewTG, Canvas3D canvas, BranchGroup root) {
            ...
            this.hoverPickCanvas.setMode(PickTool.GEOMETRY);
        ...
        private void updateHoverHighlight() {
            ...
            Shape3D hovered = null;
            if (result != null) {
                Node node = result.getNode(PickResult.SHAPE3D);
                if (node instanceof Shape3D) {
                    hovered = (Shape3D) node;
                }
            }

            for (int i = 0; i < puzzle1Answers.length; i++) {
                if (puzzle1Answers[i] == null)
                    continue;
                if (puzzle1Answers[i] == hovered) {
                    puzzle1Answers[i].setAppearance(answerHighlightAppearance());
                } else {
                    puzzle1Answers[i].setAppearance(answerNormalAppearance());
                }
            }
        }
    ...
    // ── material helpers ──────────────────────────────────────────────────────
    // chosenIndex: -1 = no answer yet, 0-3 = index chosen (1 is correct answer)
    private Appearance puzzlePaperAppearance(int chosenIndex) {
        ...
        for (int i = 0; i < 4; i++) {
            if (chosenIndex >= 0) {
                if (i == chosenIndex) {
                    // The answer the player clicked — green ✓ or red ✗
                    boolean correct = (i == correctIdx);
                    g.setColor(correct ? new Color(30, 120, 35) : new Color(180, 30, 30));
                    g.setFont(new Font("Serif", Font.BOLD, 46));
                    g.drawString(correct ? "\u2713" : "\u2717", 110, labelYs[i]);
                    g.setFont(new Font("Serif", Font.BOLD, 34));
                } else if (i == correctIdx && chosenIndex != correctIdx) {
                    // If a wrong answer was chosen, reveal the correct one with a faint green ✓
                    g.setColor(new Color(30, 120, 35, 130));
                    g.setFont(new Font("Serif", Font.BOLD, 46));
