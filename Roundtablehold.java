...
public class RoundtableHold extends JFrame {
    ...
    // quiz puzzle
    // puzzle paper is stored so we can redraw it with a check/x mark when answered
    ...
    // stray chair puzzle
    // stray chair sits pulled out from the table, all others are tucked under.
    // player must look at the stray chair and press E to push it under
    ...
    // world position of the stray chair (pulled out from seat i=2, angle=2pi/6*2)
    private static final double STRAY_ANGLE = Math.PI * 2.0 / 6.0 * 2.0; // 120 degrees
    ...
    private void addLights(BranchGroup root) {
        ...
        PointLight gem = new PointLight(YELLOW,
                new Point3f(0f, GEM_Y, 0f),
        ...
        for (int fi = 0; fi < 3; fi++) {
            PointLight fl = new PointLight(fdCol[fi],
    ...
    private TransformGroup buildFireplace() {
        ...
        for (int fi = 0; fi < 3; fi++) {
            ...
            // start behind wall since fireOn = false initially
    ...
    // this key is separate only because I (alex) was making the fireplace in a
    // branch separate from brodey's branch when we came up with the idea to have
    // the keys rise from the ground
    ...
    private void solvePuzzle2() {
        ...
        new Thread(() -> {
            ...
            // once the chair is fully tucked, move the key up
            spawnKey(1);
        }, "chair-push").start();
    }
